  ...
  [LOAD_RESOURCES] : (state, action) => {
    ...
    let findTrait = (traitName) => {
      ...
      if (typeof(traitName) === "string") {
        foundTrait = allTraits.find((trait) => (traitName === trait.name))
        ...
        keys.shift()
        let replaceTrait = allTraits.find((trait) => traitName.name === trait.name)
